...
const Game: React.FC = (props) => {
  ...
  useEffect(() => {
    axios.get(`https://localhost:7073/api/User/${props.match.params.id}`)
      .then(res => {
        setBalance(res.data[0].currency);
      })
      .catch(error => {
        console.log(error)
      })
  ...
  const placeBet = (amount: number) => {
    ...
    axios.put(`https://localhost:7073/api/User/${props.match.params.id}/newcurrency/${Math.round((balance - amount) * 100) / 100}`)
      .then(res => {
  ...
  const checkWin = () => {
    if (userScore > dealerScore || dealerScore > 21) {
      ...
      axios.put(`https://localhost:7073/api/User/${props.match.params.id}/newcurrency/${Math.round((balance + (bet * 2)) * 100) / 100}`)
        .then(res => {
          console.log("balance changed")
        })
        .catch(error => {
      ...
      axios.put(`https://localhost:7073/api/User/${props.match.params.id}/newcurrency/${Math.round((balance + (bet * 1)) * 100) / 100}`)
        .then(res => {
          console.log("balance changed")
        })
        .catch(error => {
